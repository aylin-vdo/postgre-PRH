...
CREATE TABLE Degrees (
    ...
    UserCreation INTEGER REFERENCES Users(UserID) NOT NULL,
    CreatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    UserUpdate INTEGER REFERENCES Users(UserID) NOT NULL,
    UpdatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    Active BOOLEAN DEFAULT TRUE NOT NULL
);
...
CREATE TABLE Advisors (
    ...
    Name VARCHAR(255) NOT NULL,
    DegreeIdentity INTEGER REFERENCES Degrees(Identity) NOT NULL,
    UserCreation INTEGER REFERENCES Users(UserID) NOT NULL,
    CreatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    UserUpdate INTEGER REFERENCES Users(UserID) NOT NULL,
    UpdatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
CREATE TRIGGER update_degrees_timestamp
    BEFORE UPDATE ON Degrees
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER update_advisors_timestamp
    BEFORE UPDATE ON Advisors
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER update_advisees_timestamp
    BEFORE UPDATE ON Advisees
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER update_learning_units_timestamp
    BEFORE UPDATE ON LearningUnits
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER update_advisory_sessions_timestamp
    BEFORE UPDATE ON AdvisorySessions
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER update_entry_exit_record_timestamp
    BEFORE UPDATE ON EntryExitRecord
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at();